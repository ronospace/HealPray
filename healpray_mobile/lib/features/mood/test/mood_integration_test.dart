...
class MoodIntegrationTest {
  ...
  static Future<void> _testBasicMoodOperations() async {
    ...
    print(
        '   ✓ Found ${prayerEntries.length} entries with grateful emotion and score >= 7');
  ...
  static Future<void> _testMoodAnalytics() async {
    ...
    print(
        '   ✓ Top emotion: ${stats.emotionFrequency.keys.firstOrNull ?? 'None'}');
  ...
  static Future<void> _testMoodExport() async {
    ...
    print(
        '   ✓ Analytics report generated (${reportExport.split('\\n').length} lines)');
    ...
    print(
        '   ✓ Complete export package created (${exportPackage.length} files)');
