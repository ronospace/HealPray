...
class MoodExporter {
  ...
  static String exportAnalyticsReport({
    ...
    buffer.writeln('-' * 20);
    buffer.writeln(
        'Average Mood Score: ${stats.averageScore.toStringAsFixed(1)}/10');
    buffer.writeln(
        'Median Mood Score: ${stats.medianScore.toStringAsFixed(1)}/10');
    ...
    for (int score = 10; score >= 1; score--) {
      ...
      final percentage =
          entries.isNotEmpty ? (count / entries.length * 100) : 0;
      final bar = '█' * (percentage ~/ 2);
      buffer.writeln(
          '$score/10: ${count.toString().padLeft(3)} entries (${percentage.toStringAsFixed(1)}%) $bar');
    }
    buffer.writeln();
    ...
    for (final entry in topEmotions) {
      final percentage =
          entries.isNotEmpty ? (entry.value / entries.length * 100) : 0;
      buffer.writeln(
    ...
    for (int day = 1; day <= 7; day++) {
      ...
      if (count > 0) {
        buffer.writeln(
            '${dayNames[day - 1]}: ${average.toStringAsFixed(1)}/10 ($count entries)');
      }
    }
    buffer.writeln();
    ...
    buffer.writeln('-' * 15);
    buffer.writeln(
        'Volatility Level: ${volatility.volatilityLevel.name.toUpperCase()}');
    buffer.writeln(
        'Standard Deviation: ${volatility.standardDeviation.toStringAsFixed(2)}');
    buffer.writeln(
        'Average Daily Change: ${volatility.averageChange.toStringAsFixed(1)} points');
    ...
    if (patterns.isNotEmpty) {
      ...
      for (final pattern in patterns.take(5)) {
        buffer.writeln(
            '• ${pattern.description} (Confidence: ${(pattern.confidence * 100).toStringAsFixed(0)}%)');
      }
      buffer.writeln();
    }
    ...
    buffer.writeln('=' * 50);
    buffer.writeln(
        'Report generated by HealPray - Your Spiritual Wellness Companion');
  ...
  static Map<String, dynamic> _generateAnalyticsData(
    ...
    final emotionCorrelation =
        MoodAnalytics.analyzeEmotionCorrelations(entries);
    final volatility = MoodAnalytics.calculateMoodVolatility(entries);
    final patterns = MoodAnalytics.identifyPatterns(entries);
    ...
    return {
      ...
      'emotion_analysis': {
        ...
        'common_pairs': emotionCorrelation.commonPairs
            .map((pair) => {
