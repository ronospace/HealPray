...
class ChatService {
  ...
  Future<void> initialize() async {
    try {
      ...
      AppLogger.error('Failed to initialize ChatService', e);
      rethrow;
    }
  }
  ...
  Future<Conversation> startConversation({
    ...
    AppLogger.info('Started conversation: ${conversation.id} with context: $context');
    return conversation;
  }
  ...
  Future<ChatMessage> _generateAIResponse(
    ...
    try {
      ...
      final recentMessages = messages.takeLast(10).toList(); // Last 10 messages for context
...
SPIRITUAL COMPANION: ''';
      ...
      final responseText = response.text ?? 'I\'m here to listen and support you.';
  ...
  Future<void> _sendWelcomeMessage(
    ...
    switch (context) {
      ...
      case ChatContext.prayer:
        welcomeMessage = 'Welcome to our prayer space. Whether you\'d like to pray together, need prayer guidance, or want to share prayer requests, I\'m here to support your connection with the divine.';
        break;
      ...
      case ChatContext.crisis:
        welcomeMessage = 'I\'m here to provide immediate spiritual support during this difficult time. You are loved and not alone. If you\'re in immediate danger, please contact emergency services. How can I help bring you comfort right now?';
        break;
        
      case ChatContext.meditation:
        welcomeMessage = 'Welcome to our meditation space. I\'m here to guide you toward inner peace and spiritual connection. Would you like a guided meditation, breathing exercise, or spiritual reflection?';
        break;
        
      case ChatContext.guidance:
        welcomeMessage = 'I\'m honored to offer spiritual guidance for whatever you\'re facing. Whether it\'s a decision, challenge, or question about your spiritual path, I\'m here to listen and share wisdom.';
        break;
  ...
  Map<String, dynamic> getChatInsights() {
    ...
    return {
      ...
      'contextBreakdown': contextCounts.map((k, v) => MapEntry(k.toString(), v)),
      'dailyActivity': dailyCounts,
      'averageMessagesPerConversation': conversations.isEmpty ? 0 : allMessages.length / conversations.length,
      'generatedAt': DateTime.now().toIso8601String(),
    };
  }
  ...
  void switchConversation(String conversationId) {
    ...
    final activeSession = _repository.getActiveSessionForConversation(conversationId);
    if (activeSession != null) {
      ...
      _repository.createSession(
        conversationId: conversationId,
        context: _repository.getConversation(conversationId)?.context ?? ChatContext.general,
      ).then((session) {
        _currentSessionId = session.id;
      });
    }
